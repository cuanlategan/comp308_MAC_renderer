...
void main() {
	...
	float displacement = amplitude * texture2D(texture0, gl_MultiTexCoord0.xy).r; // 0..1
	vec4 new_position = gl_Vertex + vec4(0,displacement,0,0);

	// Transform and pass on the normal/position/texture to fragment shader
	vNormal = normalize(gl_NormalMatrix * gl_Normal);
	vPosition = vec3(gl_ModelViewMatrix * new_position);
