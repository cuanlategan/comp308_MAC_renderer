...
void RiverHandler::drawAll() {
    ...
    cout << "Found " << graph->getTriangles().size() << " triangles, with " << graph->getTriEdges().size() <<
    " edges." << endl;
...
void RiverHandler::drawPoints(vector<vVertexPoint *> points, CImg<unsigned char> *pointDisplay,
                              const unsigned char color1[], const unsigned char color2[], int radius) {
...
void RiverHandler::drawRivers(vector<vector<vVertexPoint *>> riverSet, CImg<unsigned char> *pointDisplay,
                              const unsigned char lineColor[], const unsigned char nodeColor[], int radius) {
...
void RiverHandler::drawRiverSplines(vector<vector<vVertexPoint *>> riverSet, CImg<unsigned char> *pointDisplay,
                                    const unsigned char lineColor[], const unsigned char nodeColor[], int radius) {
    for (vector<vVertexPoint *> river : riverSet) {
...
void RiverHandler::drawPolygons(vector<vTriangle *> polys, CImg<unsigned char> *pointDisplay,
                                const unsigned char lineColor[], const unsigned char nodeColor[], int radius) {
    ...
    for (vTriangle *p : polys) {
        ...
        if (p->isRiver()) {
            ...
            pointDisplay->draw_circle(p->getCenter()->screenCoords.x, p->getCenter()->screenCoords.y, radius,
                                      nodeColor);
        }
...
Geometry *RiverHandler::makeGeo() {
    ...
    for (vTriangle *t : graph->getTriangles()) {
        ...
        // Changed these to make geometry align with grass
